...
note-hygiene), each with the file and the suggested fix. Do NOT edit anything — this is read-only; offer to
fix on request. For a large tree, delegate the per-doc verification to the `doc-auditor` subagent in
parallel.
